  ...
  ({
    ...
    const firstImageData = useMemo(() => {
      const firstImage = card.attachments?.find((attachment) => {
        ...
        if (
          attachment.url?.match(/\.(jpg|jpeg|png|gif|webp|svg|bmp|tiff|ico)$/i)
        ) {
          return true;
        }
        // Check by filename
              ...
              <div className="flex-1 min-w-0 relative">
                ...
                >
                  <div className="min-w-0">
                    <h4
                      className={`font-sm text-[#292a2e] text-medium leading-tight mb-1 line-clamp-5 break-words pr-5`}
                    >
                      {card.title}
                    </h4>
